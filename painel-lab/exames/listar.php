...
<table class="table table-sm mt-3">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from labexame order by id desc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from labexame where descricao LIKE '$txtbuscar' order by id desc");
	...
	//TOTALIZAR OS REGISTROS PARA PAGINAÇÃO
		$res_todos = $pdo->query("SELECT * from labexame");
